...
if ($timenow < $ex_date) {
    if ($result) {
        ...
        if (isset($_POST['Password'])) {
            ...
            if (strlen($pass) >= 8) {
                ...
                if ($uppercase) {
                    ...
                    if ($lowercase) {
                        if (!$number) {
                            $pass_error = 'Mật khẩu phải chứa một số';
                        }
                    } else {
                        $pass_error = 'Password must contain a lowercase character';
                    }
                } else {
                    $pass_error = 'Mật khẩu phải chứa ký tự chữ thường';
                }
            } else {
                $pass_error = 'Mật khẩu cần dài ít nhất 8 ký tự';
            }
        }
        if (isset($_POST['Password']) && isset($_POST['RePassword'])) {
            ...
            if ($repass != $pass) {
                $repass_error = 'Nhập lại nhập lại không đúng';
            }
        }
                ...
                <div class="card card-lg" style="border-radius: 1rem;">
                    <div class="row g-0">
                        <div class="col-md-6 col-lg-5 d-none d-md-block pt-5">
                            <img src="./assets/img/login.svg" alt="login form" class="img-fluid rounded mx-auto d-block"
                            ...
                            <div class="card-body p-4 p-lg-5 text-black">
                                <form action='index.php?action=RestPass&act=Submit' method='post'>
                                    <div class="d-flex align-items-center mb-3 pb-1">
                                        ...
                                        <span class="h1 fw-bold mb-0">AETSHOP</span>
                                    </div>
                                    <h5 class="fw-normal mb-3 pb-3" style="letter-spacing: 1px;">Vui lòng nhập lại mật
                                        khẩu mới
                                    </h5>
                                    <!-- Password input -->
                                    ...
                                    <div class="form-outline mt-4">
                                        <input type="password" id="Password"
                                            class="form-control <?php echo $pass_error != '' ? 'error-input' : '' ?>"
                                            name="Password" required value="<?php echo $pass ?>" />
                                    ...
                                    <div class="form-outline mt-4">
                                        <input type="password" id="RePassword"
                                            class="form-control <?php echo $repass_error != '' ? 'error-input' : '' ?>"
                                            name="RePassword" required value="<?php echo $repass ?>" />
                                    ...
                                    <div class="custom-control custom-checkbox form-outline mb-4">
                                        <input type="checkbox" class="custom-control-input" id="defaultUnchecked"
                                            onclick="SnH_Ps()">
                                        <label class="custom-control-label" for="defaultUnchecked" id="ShowHidePs">Hiện
                                            mật khẩu</label>
                                    </div>
                                    <div class="pt-1 mb-4">
                                        <button class="btn btn-dark btn-lg btn-block" type="submit">Đổi mật khẩu
                                            mới</button>
                                    </div>
                                </form>
                            </div>
